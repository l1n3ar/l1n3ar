...
export function Sidebar({ about, history, recs }: { about: string; history: WorkHistoryEntry[]; recs: Recommendation[] }) {
  ...
  return (
    <div className="gz-scroll border-r border-g py-4 px-6 flex flex-col min-h-0 overflow-auto">
      <div className="font-heading italic text-0_8 text-g mb-1.5">about</div>
      <p className="text-0_8 leading-relaxed mb-4 pb-3.5 border-b border-g/25">{about}</p>

      <div className="font-heading italic text-0_8 text-g mb-1.5">history</div>
      <div>
        {history.map((w) => (
          <div key={w.org} className="py-1.5 border-b border-g/14">
            <div className="flex justify-between gap-2.5">
              <span className="text-0_9 leading-tight">{w.org}</span>
              <span className="font-heading italic text-0_7 text-ink/48 whitespace-nowrap shrink-0">{w.range}</span>
            </div>
            <div className="font-heading italic text-0_8 text-g">{w.role}</div>
          </div>
        ))}
      </div>

      <div className="font-heading italic text-0_8 text-g mt-4 mb-1.5 pt-3 border-t border-g/25">recommendations</div>
          ...
          return (
            <div key={r.who} className="mb-3 pb-2.5 border-b border-g/12">
              ...
              <div className="flex items-baseline gap-2.5">
                <span className="font-heading italic text-0_7 text-ink/50">— {r.who}</span>
                {long && (
                  <button
                    type="button"
                    onClick={() => recDialogRef.current?.open(r.quote, r.who)}
                    className="font-heading italic text-0_7 text-g underline"
                  >
                    read more
                  </button>
                )}
